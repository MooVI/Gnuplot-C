...
void Gnuplot::set_terminal_std(const std::string &type)
{
    ...
    if (type.find("wxt") != std::string::npos && getenv("DISPLAY") == NULL)
    {
...
//------------------------------------------------------------------------------
//
// Plots a 1d graph from a Plotter Struct Data
//
Gnuplot& Gnuplot::plotfile_x_general(const std::vector<PlotterData>& data)
{
    //
    // check if file exists
    //
    std::vector<PlotterData>::const_iterator it;
    for (it = data.begin();it!= data.end();++it)
        file_available(it->input);
    ...
    if (it!=data.begin()) cmdstr << ", ";    
    cmdstr << "\"" << it->input << "\" using " << it->columns[0];

    if (it->title == "")
        cmdstr << " notitle ";
    else
        cmdstr << "title \"" << it->title << "\" ";

     cmdstr << "with " << it->style;
     }
    //
    // Do the actual plot
    //
...
//------------------------------------------------------------------------------
//
// Plots a 2d graph from a Plotter Struct Data
//
Gnuplot& Gnuplot::plotfile_xy_general(const std::vector<PlotterData>& data)
{
    //
    // check if file exists
    //
    ...
    if (it!=data.begin()) cmdstr << ", ";    
    cmdstr << "\"" << it->input << "\" using " << it->columns[0] << ":" << it->columns[1];
...
// Plots a 2d graph from a Plotter Struct Data
//
Gnuplot& Gnuplot::plotfile_xyz_general(const std::vector<PlotterData>& data)
{
    //
    // check if file exists
    //
...
//
std::string Gnuplot::get_tmpfile_name(){
